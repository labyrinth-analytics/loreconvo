#!/bin/bash
# ConvoVault SessionStart hook - auto-loads recent session context
...
INPUT=$(cat)
echo "[$(date)] Session load: $(echo "$INPUT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('session_id','?'))" 2>/dev/null)" >> ~/.convovault/hook.log 2>/dev/null

# stdout from auto_load.py goes to Claude Code (injected as context)
# stderr goes to hook.log for debugging
echo "$INPUT" | PYTHONPATH="$PLUGIN_ROOT/src" "$PYTHON" "$PLUGIN_ROOT/hooks/scripts/auto_load.py" 2>> ~/.convovault/hook.log
